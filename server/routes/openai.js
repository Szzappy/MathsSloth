...
router.post("/ask", async (req, res) => {
  try {
    ...
    // 3. Get user's previous hints (helpful ones only) for similar topics
    const previousHintsQuery = await pool.query(`
      ...
      WHERE uh.userid = $1 
        ...
        AND uh.helpful = 3
      GROUP BY uh.hintid, uh.hint_text, uh.helpful, q.difficulty
...
function buildUserProfile(mastery, previousHints) {
  ...
  // Analyse hint patterns from helpful hints
...
function buildHintPrompt(params) {
  const { question, difficulty, totalMarks, topics, studentAttempt, previousHintsThisQuestion, userProfile } = params;
...
router.post("/rate", async (req, res) => {
  try {
    const { hintId, helpful } = req.body; // helpful: 1, 2, or 3
