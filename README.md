...
- **97% grading agreement** between AI-assigned marks and human tutors across 200+ responses, with a multi-stage LLM evaluation pipeline using adaptive strictness calibration and contextual prompting.
- **Four-factor recommendation algorithm** combining Glicko-2 uncertainty modelling, FSRS scheduling, mastery analysis and exam weighting to dynamically personalise revision sessions.
- **Full-stack product ownership** — React, Node.js, Express, PostgreSQL, AWS, Vercel/Render/Neon, with JWT and Passport.js Google OAuth 2.0 authentication, deployed to production.
- **Analytics dashboards** with predicted grades, confidence calibration, learning velocity tracking, heatmaps and longitudinal ELO progression visualisations.
- **User-validated design**: 40+ participants in an in-person evaluation session praised the application for UI/UX, effectiveness at enhancing study and engaging performance tracking.
- **Grounded in research**: every major design decision traces back to specific findings in cognitive psychology, psychometrics and educational technology across 35 academic sources.
...
Maths Sloth closes that gap. Developed as a full-stack, RESTful adaptive learning platform, it integrates AI-driven grading and hint generation with personalised question selection. Every quiz session is generated by a four-factor recommendation algorithm that decides which of 73 A-level topics a student most needs to review right now, weighing FSRS scheduling urgency, Glicko-2 uncertainty, relative mastery deficit and exam weighting. Answers are graded in one of three formats, including free-text Feynman-style explanations marked by an LLM calibrated to the student's own ability level via adaptive strictness bands. Every attempt feeds back into the scheduling and rating engine atomically at the database layer.
...
| Metric | Result |
|---|---|
| AI grading agreement with human tutors | 97% across 200+ responses with multi-stage LLM evaluation pipeline |
| User feedback (in-person evaluation) | 40+ participants praised UI/UX, effectiveness at enhancing study, and performance tracking |
...
| Analytics features | Predicted grades, confidence calibration, learning velocity, heatmaps, longitudinal ELO visualisations |
| Authentication | JWT + Passport.js Google OAuth 2.0 with email verification |
...
- **Full-stack product ownership**: Developed a full-stack RESTful adaptive learning platform using React, Node.js, Express and PostgreSQL, with deployment to Vercel, Render, Neon and AWS. Integrated AI-driven grading and hint generation with personalised question selection.
- **LLM integration and prompt engineering**: Designed a multi-stage LLM evaluation pipeline (GPT-4o-mini) with adaptive strictness calibration and contextual prompting, achieving 97% grading agreement with human tutors across 200+ responses.
- **Algorithm design**: Designed a four-factor recommendation algorithm combining Glicko-2 uncertainty modelling, FSRS scheduling, mastery analysis and exam weighting to dynamically personalise revision sessions.
- **Database engineering**: Built an 11-table PostgreSQL schema with atomic PL/pgSQL triggers, CHECK constraints and cascading referential integrity.
- **Product evaluation and user research**: Collected user feedback from a 40+ participant in-person evaluation session, with participants praising the application for UI/UX, effectiveness at enhancing study and engaging performance tracking.
- **Authentication and deployment**: Implemented JWT and Passport.js Google OAuth 2.0 authentication with CI/CD pipelines via Vercel auto-build.
...
- BSc (Hons) Computer Science, First Class Honours, University of Warwick (2023–2026)
- MSc Applied Artificial Intelligence, University of Warwick (2026–2027)
- Freelance tutor: Oxbridge and Russell Group admissions coaching for Computer Science and Mathematics
